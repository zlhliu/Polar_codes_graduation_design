...
int main() {
    ...
    for(int i=0;i<total_bits/2;++i) // initialize frozen channel
        frozen_channel.insert(Q[i]);
    int signal_power=1; // signal power
    int num_snr=30; 
    std::vector<double> snr(num_snr); // snr=10log10(signal_power/noise_power)
    ...
    for(int times=0;times<num_snr;times++){
        ...
        decoder my_decoder(std::move(my_demodulation.result()),"sc",frozen_channel);
